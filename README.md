...
| 目录                    | 功能       | 技术栈                                           | 说明                                       |
| :---------------------- | :--------- | :----------------------------------------------- | :----------------------------------------- |
| **`yisu-hotel/mobile`** | 移动端应用 | [Taro](https://taro.zone/) + React + TypeScript  | 编译为**微信小程序**。                     |
| **`yisu-hotel/admin`**  | 管理后台   | [Vite](https://vitejs.dev/) + React + TypeScript | 用于酒店管理员管理房型、订单、用户等数据。 |
...
# 安装各子项目依赖（目前需分别进入各目录安装，并未配置 workspace）
cd yisu-hotel/mobile && pnpm install && cd ../..
cd yisu-hotel/admin && pnpm install && cd ../..
cd server && pnpm install && cd ..
```

### 2. 📱 移动端开发指南 (`yisu-hotel/mobile`)

#### 启动开发服务器

```bash
cd yisu-hotel/mobile
pnpm dev:weapp
```
...
3.  **目录**: 选择项目中的 `yisu-hotel/mobile` 文件夹 (注意**不是** `dist`)。
4.  **AppID**: 使用您的测试号或正式 AppID。
5.  **后端服务**: 选择“不使用云服务”。
6.  点击 **确定**。
7.  进入后，确保在详情设置中**关闭** ES6 转 ES5、样式补全等（交给 Taro 处理）。

### 3. 💻 管理后台开发指南 (`yisu-hotel/admin`)
